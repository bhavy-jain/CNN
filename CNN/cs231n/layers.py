...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
        cache=(xnorm, gamma,np.sqrt(sample_var+eps),x-sample_mean)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    dxnorm=dout*gamma
    dxmu1=dxnorm/std
    dinvstd=np.sum(dxnorm*xmu,axis=0)
    dstd=dinvstd*(-1/std**2)
    dvar=dstd/(2*std)
    dsq=(dvar*np.ones(xnorm.shape))/xnorm.shape[0]
    dxmu2=2*dsq*xmu
    dmu=np.sum(-1*(dxmu1+dxmu2),axis=0)
    dx1=dxmu1+dxmu2
    dx2=(dmu*np.ones(xnorm.shape))/xnorm.shape[0]
    dx=dx1+dx2
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def batchnorm_backward_alt(dout, cache):
    ...
    dgamma=np.sum(dout*xnorm,axis=0)
    dbeta=np.sum(dout,axis=0)
    dx=(1/xnorm.shape[0])*(gamma/std)*(xnorm.shape[0]*dout-np.sum(dout,axis=0)-xmu*(np.sum(dout*xmu, axis=0))/(std**2))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    ...
    for n in range(N):
        ...
        for i in range(0,Hdash):
            for j in range(0,Wdash):
                x_col.append(x_pad[n,:,i*stride:i*stride+HH,j*stride:j*stride+WW].reshape(-1,1))
        x_col=np.array(x_col)
        out.append(np.dot(w.reshape(F,-1),x_col).reshape(F,Hdash,Wdash)+b.reshape(-1,1,1))
    out=np.array(out)'''
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for i in range(Hdash):
                for j in range(Wdash):
                    p=np.argmax(x[n,c,i*stride:i*stride+HH,j*stride:j*stride+WW])
                    dx[n,c,i*stride+p//WW,j*stride+p%WW]=dout[n,c,i,j]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
